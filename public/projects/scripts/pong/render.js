...
class Renderer {
    ...
    renderGame(){
        ...
        gameScreens.score(gameObjects.player1.pts, gameObjects.player2.pts, gameCanvas.canvas.width/2, bounds.height/6, gameCanvas);

        Object.keys(gameObjects).forEach( key => {
            gameObjects[key].draw(gameCanvas);
        });
    }    
}
...
let gameScreens = {
    ...
    score(p1Score, p2Score, centerX, centerY, canvasContext){
        ...
        if(p1Score >= WINNINGSCORE){
          let p1TOcent = centerX - (gameObjects.player1.x + gameObjects.player1.width);
          p1TOcent *= .9;
    ...
    score_narrow(p1Score, p2Score, centerX, centerY, canvasContext){
        //let {offsetY, bounds} = getGameObjects();
        let {offsetY, ...bounds} = getWindowConfig();
