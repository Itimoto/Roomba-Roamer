...
var pongURL = `wss://${document.location.hostname}/pong`;
...
window.onload = (event) => {
    var pongCanvas = document.getElementsByClassName("pongCanvas")[0];
    pongCanvas.width = window.innerWidth;   // We resize it in here b/c resizing w/ CSS results in some nasty blur on the Canvas (when using it to draw rects, etc.)
    pongCanvas.height = window.innerHeight;
    
    var pongClient; // null until initialized
    var requestedPong = false;
    
    pongWS.addEventListener('open', function (event) {
